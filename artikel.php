...
<body>
    ...
    while($row = mysql_fetch_assoc($query)) {
        ...
        $commentquery = mysql_query("SELECT * FROM comment WHERE blogid = '$artikelId' ");
        while($row = mysql_fetch_assoc($commentquery)) {
        $commentId = $row['id'];
        $name = $row['name'];
        $comment = nl2br($row['comment']);
        $blogId = $row['blogid'];
        $createdate = $row['posted'];

        echo "
            <div id='comment'>
            <br/>
            <h4>{$name}</h4>
            <p>schrieb am {$createdate}:</p>
            <p> $comment </p>
            </div>
        "; 
         } 
         ...
         echo "
            <div id='eingabe'>
                <form action='posten.php'>
                        <p>Name:</p> <input name='name' type='text' /> <br />
                        <p>Kommentar:</p> <textarea name='comment' cols='35' rows='10'> </textarea> <br />
